...
def fraction(folder):
    if folder == "upstat": return upstat("nominal")
    if folder == "downstat": return downstat("nominal")
...
def upstat(folder):
    ...
    fractions = OrderedDict()
    for i in range(1, h.GetNbinsX()+1):
        fractions[h.GetXaxis().GetBinLabel(i)] = (h.GetBinContent(i) + h.GetBinError(i)) / total
    return fractions
...
def table():
    ...
    for sys in "upstat", "downstat", "scaleup", "scaledown", "tuneup", "tunedown":
